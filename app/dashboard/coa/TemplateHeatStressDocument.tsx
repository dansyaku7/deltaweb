...
export const TemplateHeatStressDocument = React.forwardRef<HTMLDivElement, TemplateProps>(
  ({ data }, ref) => {
    ...
    return (
      <div ref={ref} className="p-10 font-serif text-black bg-white relative" style={{ width: '210mm', minHeight: '297mm' }}>
        ...
        <div className="relative z-10 flex flex-col h-full">
            ...
            <table className="w-full border-collapse border-2 border-black mb-4">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="border border-black p-1 text-center">{data.results.length > 1 ? '(See Table)' : (data.results[0]?.location || '-')}</td>
                  <td className="border border-black p-1 text-center">Heat Stress</td>
                  <td className="border border-black p-1 text-center">{sampleInfo.samplingDate || '-'}</td>
                  <td className="border border-black p-1 text-center">{data.results.length > 1 ? '(See Table)' : (data.results[0]?.time || '-')}</td>
          ...
          <footer className="mt-100 pt-8 text-[7px] italic">
